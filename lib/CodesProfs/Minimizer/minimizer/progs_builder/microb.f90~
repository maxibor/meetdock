...
!==================================
  ...
  part1=prot_id(2)%Ca_item(1)
  part2=prot_id(2)%Ca_item(5)
  ...
  write(6,form_tens) 'current angles alpha/beta/gamma:', alpha, beta, gamma

  return

end subroutine start_rotation

subroutine rotation(position,prot,init)
!=====================================
  ...
  real(8),dimension(6) :: dlig,npos

  Npart=prot_id(prot)%number

  npos(1)=center(1)%x+position(1)*sin(position(2))*cos(position(3))
  npos(2)=center(1)%y+position(1)*sin(position(2))*sin(position(3))
  npos(3)=center(1)%z+position(1)*cos(position(2))

  npos(4:6)=position(4:6)
  ...
  do i=1,Npart
    ...
    part=prot_id(prot)%item(i)

    xi=old_position(part)%x-Old_Lig(1)
    yi=old_position(part)%y-Old_Lig(2)
    zi=old_position(part)%z-Old_Lig(3)
...
! rotation de dgamma 
    ...
    molecule(part)%x=npos(1)+x3i
    molecule(part)%y=npos(2)+y3i
    molecule(part)%z=npos(3)+z3i
